...
public class ScratchTest2 {
	...
	public static void main(String[] args) throws Exception {
		InputStream in = Thread.currentThread().getContextClassLoader().getResourceAsStream("test-data/支出.xls");
		Map<BigDecimal, SortedSet<Integer>> m1 = loadExcel(in, 0);

		in = Thread.currentThread().getContextClassLoader().getResourceAsStream("test-data/预付0414.xls");
		Map<BigDecimal, SortedSet<Integer>> m2 = loadExcel(in, 0);
		...
		in = Thread.currentThread().getContextClassLoader().getResourceAsStream("test-data/支出.xls");
		markExcel(in, "/tmp/支出.xls", 0, m1);
		...
		in = Thread.currentThread().getContextClassLoader().getResourceAsStream("test-data/预付0414.xls");
		markExcel(in, "/tmp/预付0414.xls", 0, m2);
	}
